...
"Πότε επιτρέπεται ο Στρατός να επέμβει σε αστικό περιβάλλον?","Πώς διαχειρίζεσαι εντολή ανωτέρου που θεωρείς λανθασμένη?",
"Έχεις εντολή να φυλάξεις στρατιωτική βάση και πολίτης προσπαθεί να μπει. Πώς αντιδράς?", "Στρατιώτης παραβιάζει στρατιωτικό πρωτόκολλο. Πώς το αναφέρεις?"]
...
class SupportTicketSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        cfg = {
            "ownership":  (ownership_roles,         f"ownership-{author.name}",   "Ownership Ticket"),
            "ban_appeal": (donate_and_ownership,     f"ban-appeal-{author.name}",  "Ban Appeal"),
...
class JobTicketView(discord.ui.View):
    ...
    async def _create_job_ticket(self, interaction: discord.Interaction, tt: str, manager_rid: int):
        guild = interaction.guild; author = interaction.user
...
class DonateTicketView(discord.ui.View):
    ...
    async def donate(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        e.set_image(url=BANNER_DONATE)
        e.set_footer(text="Quantum Roleplay • Donate System")
        await ch.send(embed=e, view=make_ticket_close_view(author.id))
        lc = guild.get_channel(TICKET_LOG_ID)
        if lc:
...
class ReasonModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        role_map = {
            "police":   POLICE_ROLE_ID,
            "ekab":     EKAB_ROLE_ID,
            "limeniko": LIMENIKO_ROLE_ID,
            "dimarxio": DIMARXIO_ROLE_ID,
            "stratos":  STRATOS_ROLE_ID,
...
class StartApplicationView(discord.ui.View):
    def __init__(self, app_type):
        ...
        labels = {
            "police":"▶️ Start Police","ekab":"▶️ Start ΕΚΑΒ","limeniko":"▶️ Start Λιμενικό",
    ...
    async def start_btn(self, interaction, button):
        ...
        qs_map = {
            "police":POLICE_QUESTIONS,"ekab":EKAB_QUESTIONS,"limeniko":LIMENIKO_QUESTIONS,
...
class SendApplicationView(discord.ui.View):
    ...
    async def send_btn(self, interaction, button):
        ...
        rc_map = {
            "police":POLICE_RESULTS_CHANNEL_ID,"ekab":EKAB_RESULTS_CHANNEL_ID,
        ...
        labels = {"police":"🚓 Police","ekab":"🚑 ΕΚΑΒ","limeniko":"⚓ Λιμενικό","dimarxio":"🏛️ Δημαρχείο","stratos":"⚔️ Στρατός","staff":"👮 Staff","manager":"👔 Manager"}
...
class ApplicationSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        e.set_image(url=BANNER_APP)
        e.set_footer(text="Quantum Roleplay • Applications")
...
async def on_message(message):
    ...
    if guild and TOKEN_PATTERN.search(message.content):
        ...
        if log:
            e = discord.Embed(title="🔑 TOKEN DETECTED!", description=f"{author.mention} έστειλε κάτι που μοιάζει με Bot Token!\n⚠️ **Άλλαξέ το ΑΜΕΣΩΣ!**", color=discord.Color.dark_red(), timestamp=discord.utils.utcnow())
...
async def dmall(ctx, *, message: str):
    ...
    guild=ctx.guild
    now_str=discord.utils.utcnow().strftime("%d/%m/%Y %H:%M EYR")
    sent=0; failed=0
    for m in guild.members:
        if m.bot: continue
        try:
...
async def lockapplication(ctx, app_type: str=None):
    ...
    if not app_type or app_type.lower() not in valid:
        status=""
        for t in valid[:-1]: status+=f"{'🔒' if t in locked_applications else '🔓'} **{t.capitalize()}**\n"
...
async def panel2(ctx):
    if not is_ownership(ctx.author): return await ctx.reply("❌ Μόνο Ownership.")
    e=discord.Embed(title="📌 Quantum Roleplay — Ownership Panel", color=discord.Color.gold(), timestamp=discord.utils.utcnow())
    e.set_thumbnail(url=ctx.guild.icon.url if ctx.guild.icon else None)
